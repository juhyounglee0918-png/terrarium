...
| 그룹 | 예시 종 | 모델링 방식 | 핵심 변수 |
|---|---|---|---|
| **식물** | 이끼, 양치류, 피토니아, 필레아, 소형 난 | 개체 기반 + 기관별(잎/줄기/뿌리) 탄소 풀 | 광합성, 호흡, 증산, 기공전도도, 잎 수분퍼텐셜, 영양, 스트레스, 번식(포자) |
| **분해자 동물** | 톡토기(springtail), 쥐며느리(isopod) | 톡토기: 격자 셀별 **개체군(코호트)**, 쥐며느리: **개별 에이전트** | 체온 의존 대사, O₂ 소비·CO₂ 배출, 먹이, 수분, 발육 단계, 번식, 사망 → 사체 |
| **해충/문제 생물** | 버섯파리(fungus gnat), 진드기, 달팽이 | 개체군 | 과습·유기물 과다에서 폭증 |
| **미생물** | 세균, 곰팡이(분해균), 병원성 곰팡이 | 셀별 기능 그룹 바이오매스 | 분해 속도 `k = kmax·f(T)·f(θ)·f(C:N)`, 곰팡이는 RH>~85% + 정체 공기 + 사체에서 증식 |
...
**웹 기반 (TypeScript + WebGPU)** 을 추천:
- 설치 없이 브라우저에서 바로 실행, 공유 쉬움.
- **Three.js (WebGPURenderer + TSL 노드 셰이더)**: 유리 굴절, SSS, 볼륨라이트 등 필요한 기능이 가능하고 생태계가 큼.
- 시뮬레이션은 **Web Worker**에서 렌더링과 분리 실행, 상태는 TypedArray(SoA)로 전달.
- 무거운 부분(토양 격자, 개체군)은 필요 시 **Rust → WASM** 또는 WebGPU compute로 이식.
- UI: React(또는 Solid) + 차트 라이브러리. 빌드: Vite. 테스트: Vitest.
...
terrarium/
├─ packages/
│  ├─ sim-core/          # 순수 TS, DOM 없음, 결정론적
│  │  ├─ env/            # light, thermal, air, water, soil-chem
│  │  ├─ bio/            # plant, fauna, microbe
│  │  ├─ ledger/         # 물·C·N·에너지 보존 검증
│  │  └─ scheduler/      # 다중 속도 스텝
│  ├─ species-data/      # 종 파라미터 JSON + 출처
│  ├─ renderer/          # Three.js 씬, 셰이더, 절차적 식물
│  └─ app/               # UI, 입력, 저장/불러오기
└─ docs/
```

- `sim-core`는 렌더러를 전혀 모름 → 헤드리스로 몇 년치를 몇 초 만에 돌려 **밸런스·회귀 테스트** 가능.
- 각 모듈은 `step(state, dt)` 형태의 순수 함수 + 명확한 입력/출력 플럭스.
...
| **M3 토양 생태** | 유기물, 미생물 분해, 질소 순환, 곰팡이 | 낙엽 → 분해 → 영양, 곰팡이 발생 |
| **M4 동물** | 톡토기(개체군), 쥐며느리(에이전트) | 곰팡이–톡토기 균형 |
...
1. **플랫폼**: 웹(Three.js/WebGPU) vs 게임 엔진(Unity/Unreal/Godot)?
2. **게임성 비중**: 순수 관찰 샌드박스 vs 목표·점수가 있는 게임?
3. **현실성 수준**: 논문 수준 모델(Farquhar 등) vs "그럴듯한" 단순 모델부터?
4. **첫 번째 테라리움 테마**: 열대 밀폐형(이끼·양치) vs 건조형(다육·선인장) vs 팔루다리움(물+육지)?
